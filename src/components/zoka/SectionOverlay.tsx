...
import { X, Monitor, Smartphone, Apple, Github, Play, FileText, BookOpen } from "lucide-react";
  ...
  docs: {
    ...
    body: (
      <div className="space-y-16">
        <p>
          ZOKA is a Rust blockchain with privacy by default: every value-bearing
          transaction is, by construction, opaque to any observer other than sender
          or receiver. No economic data is ever published in cleartext.
        </p>

        {/* Six technical pillars */}
        <div>
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-6">
            § 01 · Six technical pillars
          </div>
          <div className="space-y-0">
            {[
              { id: "01", name: "Groth16 zk-SNARKs", desc: "Over BLS12-381 · prove validity without revealing values" },
              { id: "02", name: "Pedersen commitments", desc: "Over Ristretto255 · hide transacted amounts" },
              { id: "03", name: "Bulletproof ranges", desc: "Prevent overflow-based inflation · ~600 B / proof" },
              { id: "04", name: "CLSAG ring signatures", desc: "Monero-style · sender hidden within candidate set" },
              { id: "05", name: "Stealth addresses", desc: "Separated scan & spend keys · one-time outputs" },
              { id: "06", name: "Anonymous transport", desc: "Tor / I2P + Dandelion++ · IP decoupled from tx" },
            ].map((l) => (
        ...
        <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { id: "01", name: "PBFT finality", desc: "Pre-Prepare · Prepare · Commit. 2f+1 signatures yield irreversible finality in < 3 s." },
              { id: "02", name: "Key-rotating PoW", desc: "RandomX-compatible design. Per-epoch key rotation every 2,048 blocks (~2.8 days). CPU-friendly, ASIC-resistant." },
            ].map((l) => (
              <div key={l.id} className="p-5 border border-border min-h-[140px] flex flex-col justify-between">
                <span className="font-mono text-[10px] tracking-[0.25em] uppercase text-muted-foreground">{l.id}</span>
                <div>
                  <div className="text-lg font-light text-foreground">{l.name}</div>
                  <div className="text-sm text-muted-foreground font-light mt-2 leading-relaxed">{l.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-6">
            § 03 · Five non-negotiable principles
          </div>
          <div className="space-y-0">
            {[
        ...
        <div>
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-6">
            § 04 · Private transaction model
          </div>
          <p className="mb-6">
            ZOKA uses a <span className="text-foreground">note model</span> (Zcash-Sapling-style, adapted to
            CLSAG). Each note carries a hidden value, a blinding factor, a spending key, an index in the
            private commitment tree and a stealth address.
          </p>
          <div className="space-y-0">
            {[
              { id: "01", name: "Note selection", desc: "Wallet picks own notes covering value + fee." },
              { id: "02", name: "CLSAG ring", desc: "Up to 10 candidates (real + decoys). Validators don't know which is real." },
              { id: "03", name: "zk-SNARK proof", desc: "Balance preserved · no double spend · valid ranges · correct ring." },
              { id: "04", name: "Pedersen outputs", desc: "Hidden amounts with Bulletproof range proofs." },
              { id: "05", name: "Stealth derivation", desc: "One-time addresses via ECDH with receiver's key." },
              { id: "06", name: "Encrypted payload", desc: "Amount and blinding factor encrypted with ECDH-derived key." },
            ].map((l) => (
              <div key={l.id} className="grid grid-cols-12 gap-4 py-4 border-b border-border">
                <div className="col-span-2 md:col-span-1 font-mono text-xs text-muted-foreground">{l.id}</div>
                <div className="col-span-10 md:col-span-4 text-base font-light">{l.name}</div>
        ...
        <div>
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-6">
            § 05 · zk-SNARK circuits — Groth16 / BLS12-381
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div key={l.id} className="p-5 border border-border min-h-[130px] flex flex-col justify-between">
                <span className="font-mono text-[10px] tracking-[0.25em] uppercase text-muted-foreground">{l.id}</span>
                <div>
                  <div className="text-base font-mono text-foreground">{l.name}</div>
                  <div className="text-sm text-muted-foreground font-light mt-2 leading-relaxed">{l.desc}</div>
                </div>
              </div>
            ))}
        ...
        <div>
          <div className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-6">
            § 08 · Fees & MEV protection
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { id: "01", name: "Private fees", desc: "Hidden under Pedersen commitment with Bulletproof range. Node-configurable floor; wallet hints low / normal / high." },
              { id: "02", name: "Private coinbase", desc: "Miner reward as private as any tx. Observers see a block was mined, not how much was earned." },
              { id: "03", name: "MEV-resistant", desc: "Hidden amounts and destinations make sandwich and front-running attacks structurally impossible." },
              { id: "04", name: "No hidden taxes", desc: "No protocol-level whitelist, blacklist, or fee on holding / sending / receiving." },
            ].map((l) => (
              <div key={l.id} className="p-5 border border-border min-h-[140px] flex flex-col justify-between">
                <span className="font-mono text-[10px] tracking-[0.25em] uppercase text-muted-foreground">{l.id}</span>
                <div>
        ...
        <div>
          ...
          <div className="space-y-0">
            {[
              { id: "01", name: "Internal devnet", desc: "ZK circuits · single-operator setup · 4–7 local validators · CLI.", status: "Done" },
              { id: "02", name: "Public testnet", desc: "MPC ceremony ≥ 5 contributors · CRS digest baked · Tor/I2P · mixer batching.", status: "Done" },
              { id: "03", name: "Mainnet", desc: "External audit · code freeze · validator bootstrap · permissionless mining.", status: "In progress" },
              { id: "04", name: "Post-mainnet", desc: "Native RandomX VM · BIP39 for private wallets · multi-asset · post-quantum migration · confidential bridges.", status: "Planned" },
            ].map((l) => (
              <div key={l.id} className="grid grid-cols-12 gap-4 py-4 border-b border-border">
                <div className="col-span-2 md:col-span-1 font-mono text-xs text-muted-foreground">{l.id}</div>
                <div className="col-span-10 md:col-span-4 text-base font-light">{l.name}</div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm font-light text-muted-foreground">
            {[
              ...
              <li key={x} className="flex gap-3 border-b border-border pb-3">
